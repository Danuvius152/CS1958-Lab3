 ...
 public:
  ...
  class iterator {
   ...
   public:
    ...
    iterator operator+(const int &n) const { return iterator(p, index + n); }
    iterator operator-(const int &n) const { return iterator(p, index - n); }
    // return the distance between two iterators,
    // if these two iterators point to different vectors, throw
    // invaild_iterator.
    int operator-(const iterator &rhs) const {
      if (this->p != rhs.p) throw invalid_iterator();
    ...
    T &operator*() const { return *(p->store[index]); }
  ...
  class const_iterator {
   ...
   public:
    const_iterator(const vector<T> *pv, long long idx)
        : constp(pv), index(idx){};
    ...
    // invaild_iterator.
    int operator-(const iterator &rhs) const {
      if (this->p != rhs.constp) throw invalid_iterator();
      return this->index - rhs.index;
    }
    ...
    T &operator*() const { return *(constp->store[index]); }
   ...
   */
  vector() : _size(0), cap(4) { store = new T *[4]; }
  vector(const vector &other) : _size(other._size), cap(other.cap) {
    store = new T *[cap];
    for (int i = 0; i < _size; i++) store[i] = new T(*(other.store[i]));
  }
  ~vector() {
    for (int i = 0; i < _size; i++) delete store[i];
    delete[] store;
  }
  /**
  ...
  void doublesize() {
    ...
    for (int i = 0; i < _size; i++) newspace[i] = store[i];
    delete[] store;
    store = newspace;
  }
  ...
  iterator insert(iterator pos, const T &value) {
    if (full()) doublesize();
    for (int i = _size - 1; i >= pos.index; i--) store[i + 1] = store[i];
    _size++;
  ...
  iterator insert(const size_t &ind, const T &value) {
    ...
    for (int i = _size - 1; i >= ind; i++) store[i + 1] = store[i];
    _size++;
  ...
  iterator erase(iterator pos) {
    ...
    for (int i = pos.index; i < _size - 1; i++) store[i] = store[i + 1];
    _size--;
    if (_size < (cap / 4)) shrink();
    return pos;
  }
  ...
  iterator erase(const size_t &ind) {
    if (ind >= _size) throw index_out_of_bound();
    delete store[index];
    for (int i = ind; i < _size - 1; i++) store[i] = store[i + 1];
    _size--;
    if (_size < (cap / 4)) shrink();
    return iterator(this, ind);
  }
